...
public class TenantProvisioningServiceImpl implements TenantProvisioningService {
    ...
    private void initializeData(String schemaName) {
        jdbcTemplate.execute(String.format("CREATE TABLE IF NOT EXISTS %s.users (id SERIAL PRIMARY KEY, username VARCHAR(255) NOT NULL, password VARCHAR(255) NOT NULL)", schemaName));
        jdbcTemplate.execute(String.format("INSERT INTO %s.users (username, password) VALUES ('admin', 'admin')", schemaName));
        jdbcTemplate.execute(String.format("CREATE TABLE IF NOT EXISTS %s.roles (id SERIAL PRIMARY KEY, name VARCHAR(255) NOT NULL)", schemaName));
        jdbcTemplate.execute(String.format("INSERT INTO %s.roles (name) VALUES ('ROLE_ADMIN')", schemaName));
        jdbcTemplate.execute(String.format("CREATE TABLE IF NOT EXISTS %s.user_roles (user_id INT NOT NULL, role_id INT NOT NULL)", schemaName));
        jdbcTemplate.execute(String.format("INSERT INTO %s.user_roles (user_id, role_id) VALUES (1, 1)", schemaName));
    }
}
